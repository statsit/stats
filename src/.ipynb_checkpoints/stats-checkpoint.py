...
class Stats:
    ...
    def remove_space_header(self) -> list:
        try:
            if type(self.df) != pd.DataFrame:
                raise ValueError('input value is not pandas dataframe')
            features = self.df.columns
            features_with_no_space = [''.join(j for j in i if not j.isspace()) for i in features]
            return features_with_no_space
        except ValueError as e:
            return e


    def check_feature_missing_values(self) -> dict:
        try:
            if type(self.df) != pd.DataFrame:
                raise ValueError('input value is not pandas dataframe')

            if_missing_value = dict(self.df.isnull().sum())
            return {col: f'{round((if_missing_value[col] / len(self.df)) * 100, 2)}%' for col in if_null}
    ...
    def upper_lower_range(self, num: int) -> dict:
        ...
        result = {col: [round((mean_dict[col] - (num * std_dict[col])), 2),
                        round((mean_dict[col] + (num * std_dict[col])), 2)] for col in std_dict}
        return result
    ...
    def remove_duplicate(self):
        ...
        if no_of_dup > 0:
            dup = len(self.df[self.df.duplicated()].drop_duplicates(keep='first'))
